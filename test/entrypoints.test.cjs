...
test('snapshot all applies all real associations in one grouped installation',t=>{const p=makeProject(t),q=makeProject(t),f=p.selection(['alpha']);ok(p.run('init','--file',f,'--target','codex,claude'));const snap=path.join(p.root,'all.yaml');ok(p.run('snapshot','--target','all','--output',snap));ok(q.run('apply',snap,'--target','all'));assert(fs.existsSync(path.join(q.path,'.claude/skills/alpha/SKILL.md')));assert.equal(q.calls().filter(c=>c.cwd===q.path&&c.args[0]==='add').length,1);});
test('conflicting sources across snapshot targets fail before writes',t=>{const p=makeProject(t);const snap=path.join(p.root,'conflict.yaml');fs.writeFileSync(snap,'schema: skillsman.snapshot.v1\ntargets:\n  codex:\n    skills:\n      - name: alpha\n        source: example/kit\n  claude-code:\n    skills:\n      - name: alpha\n        source: example/other\n');const before=treeDigest(p.path);assert.notEqual(p.run('apply',snap,'--target','codex,claude').status,0);assert.equal(treeDigest(p.path),before);assert(!p.calls().some(c=>c.args[0]==='add'));});

test('snapshot preserves a GitHub subdirectory when root and nested skills share a name',t=>{
 const p=makeProject(t),q=makeProject(t),source='example/kit/skills/alpha';
 for(const project of [p,q])project.configure({catalog:{...project.settings.catalog,[source]:{alpha:'nested intended content'}},lockSourceBySource:{[source]:'example/kit'},skillPathByName:{alpha:'skills/alpha/SKILL.md'}});
 const file=p.selection(['alpha'],source);ok(p.run('init','--file',file,'--target','codex'));
 const snapshot=path.join(p.root,'nested.yaml');ok(p.run('snapshot','--target','codex','--output',snapshot));assert.match(fs.readFileSync(snapshot,'utf8'),/source: example\/kit\/skills\/alpha/);
 ok(q.run('apply',snapshot,'--target','codex'));assert.match(fs.readFileSync(path.join(q.path,'.agents/skills/alpha/SKILL.md'),'utf8'),/nested intended content/);
});
