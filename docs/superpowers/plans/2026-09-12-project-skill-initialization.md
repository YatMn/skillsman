...
**Plan status (2026-09-21):** 用户已授权执行，任务 1–8 的产品实现和离线验收已完成，真实分发、本地及远程两项安装也已验证。当前收尾记录见[实际验收报告](../../verification/project-skill-initialization.md)。下方保留原始实施步骤作为设计记录；阶段状态以本表及验收报告为准，不把当时的示例代码或建议提交方式当作新的待办。

| 任务 | 实际交付 |
| --- | --- |
| 1 | 隔离工作区、保留当前自有源码、实际包清单及负例测试 |
| 2 | 固定上游版本、正式 YAML 解析、结构化进程合同和夹具 |
| 3 | 完整库存、链接识别、来源证据和严格快照 |
| 4 | 内容摘要、原子状态写入、明确计划及覆盖保护 |
| 5 | 文件清单预览、缺项初始化、部分添加、锁及重试 |
| 6 | 原场景语义、管理技能、可移植入口和中英文说明 |
| 7 | 显式范围更新、共享关联、预检与安装后漂移检查 |
| 8 | 分发完整性、真实上游烟测、独立审查及验收报告 |

实施细化：`update --target all` 只展开已有目标，`apply --target all` 展开快照内目标；不把两者转换成全 Agent 新安装。实现按模块整合后统一提交，未机械拆成八个中间提交。
